...
export default {
  ...
  serviceAnnouncement2026Spring: {
    title: '서비스 공지',
    content: '개발자의 개인 일정으로 인해 4월 17일부터 4월 27일까지 웹사이트의 일부 콘텐츠(쿠폰 코드, 이벤트 타임라인 등) 업데이트가 지연될 수 있습니다. 이용에 불편을 드려 죄송합니다.'
  },
  ...
  profile: {
    ...
    claimAll: '일괄 교환',
    manualClaim: '수동 입력',
    manualClaimLabel: '쿠폰 코드',
    manualClaimEmpty: '쿠폰 코드를 입력해 주세요',
  ...
  settings: {
    ...
    contentSettings: {
      ...
      assistantTitle: 'BD2 Assistant 데스크톱 펫',
      assistantDesc: '출석 체크를 깜빡하신 적이 있나요? 브라우저 확장 프로그램을 설치하여 백그라운드에서 웹숍 출석 체크를 자동으로 완료하고, 데스크톱 펫 및 쿠폰 알림을 받아보세요.',
      assistantSecurity: '보안 및 권한 안내'
    },
    notifications: {
      title: '알림 설정'
    },
...
다시 한번 여러분의 성원에 감사드립니다!`,
    assistantSecurityDialog: {
      title: '보안 및 권한 안내',
      section1Title: '1. 토큰 권한에 관한 진실',
      section1Content: '동기화 로그인 시, 확장 프로그램은 Firebase 리프레시 토큰(refreshToken)을 획득하여 브라우저 로컬에 저장합니다. 기술적으로 이 토큰을 소유하는 것은 웹숍 계정에 대한 완전한 로그인 제어권(상점 구매 등을 포함한 모든 작업)을 갖는 것과 같습니다.',
      section2Title: '2. 토큰을 보호하는 방법',
      section2Content: '확장 프로그램은 모든 자격 증명을 100% 사용자 브라우저 로컬(chrome.storage.local)에만 저장하며, 외부나 개발자 서버로 토큰을 전송하지 않습니다. 소스 코드는 GitHub에 완전히 공개되어 공개 검증을 받고 있으며, 코드에는 공식 출석 체크 API를 호출하는 로직만 포함되어 있고 구매나 결제 기능은 전혀 없습니다.',
      section3Title: '3. 권장 및 선택권',
      section3Content: '로컬 토큰 저장에 대해 우려가 있으시다면 웹숍 동기화 기능을 활성화하지 않는 것을 강력히 권장합니다. 이 기능을 활성화하지 않아도 데스크톱 펫 및 쿠폰 코드 알림 기능은 정상적으로 이용하실 수 있습니다.'
    }
  },
