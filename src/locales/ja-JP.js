...
export default {
  ...
  serviceAnnouncement2026Spring: {
    title: 'サービス告知',
    content: '開発者の個人的なスケジュールの都合により、4月17日から4月27日までの間、ウェブサイトの一部コンテンツ（クーポンコードやイベントタイムラインなど）の更新が遅れる場合があります。ご不便をおかけして申し訳ありません。'
  },
  ...
  settings: {
    ...
    contentSettings: {
      ...
      assistantTitle: 'BD2 Assistant デスクトップペット',
      assistantDesc: 'ログインボーナスの受け取りを忘れたことはありませんか？ブラウザ拡張機能をインストールすれば、バックグラウンドでウェブショップのチェックインを自動完了し、デスクトップペットやクーポン通知を受け取ることができます。',
      assistantSecurity: 'セキュリティと権限について'
    },
    notifications: {
      title: '通知設定'
    },
...
いつも温かいご支援、心より感謝申し上げます。`,
    assistantSecurityDialog: {
      title: 'セキュリティと権限について',
      section1Title: '1. トークンの権限に関する真実',
      section1Content: '同期ログインの際、拡張機能はFirebaseの更新トークン（refreshToken）を取得し、ブラウザのローカルに保存します。技術的には、このトークンを所有することは、WebShopのアカウントに対する完全なログイン制御権（ストアでの購入などを含むすべての操作）を持つことと同等です。',
      section2Title: '2. トークンをどのように保護するか？',
      section2Content: '拡張機能は資格情報を100%ローカルのブラウザ（chrome.storage.local）内に保存し、外部サーバーや開発者のサーバーに送信することは決してありません。ソースコードはGitHubで完全に公開されており、公式チェックインAPIを呼び出すロジックのみが含まれ、購入や決済のロジックは一切含まれていません。',
      section3Title: '3. 推奨と選択肢',
      section3Content: 'ローカルでのトークン保存に懸念がある場合は、ウェブショップ同期機能を有効にしないことを強くお勧めします。この機能を有効にしなくても、デスクトップペットやクーポンコードの通知機能は通常通りご利用いただけます。'
    }
  },
