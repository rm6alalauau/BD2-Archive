    ...
    <v-card-text class="flex-grow-1 profile-card-content">
      ...
      <div v-else class="coupon-display-area">
        <div class="user-profile-card">
          <v-avatar size="48" @click="avatarDialog = true" class="user-avatar">
...
const rules = computed(() => ({
  required: (value) => !!value || t.value('profile.rules.required'),
  maxLength: (max) => (value) => 
    !value || value.length <= max || t.value('profile.rules.maxLength', null, { max })
}))
